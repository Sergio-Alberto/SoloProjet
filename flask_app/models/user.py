...
class User:
    ...
    @classmethod
    def get_by_alias(cls,data):
        query = "SELECT * FROM user WHERE alias = %(alias)s;"
        results = connectToMySQL(cls.db_name).query_db(query,data)
        if len(results) < 1:
            return False
        return cls(results[0])
    ...
    def get_all_likes_with_user( cls , data):
        query = "SELECT * FROM user LEFT JOIN bright_ideas.likes ON likes.user_id = user.id WHERE user.id =%(id)s;"
        results = connectToMySQL(cls.db_name).query_db( query , data )
        this_user = cls(results[0])
        for row in results: 
            l_info = {
    ...
    def validate_register(user):
        ...
        if not EMAIL_REGEX.match(user['email']):
            ...
            is_valid=False
        if len(results) >= 1:
            flash("Correo electrónico ya utilizado.","register")
            is_valid=False
        query = "SELECT * FROM user WHERE alias = %(alias)s;"
        results = connectToMySQL(User.db_name).query_db(query, user)
        if len(results) >= 1:
            flash("Alias ya utilizado.","register")
            is_valid=False
        if len(user['name']) < 2:
            flash("Nombre y Apellido debe tener al menos dos caracteres","register")
            is_valid= False
        if len(user['alias']) < 2:
            flash("alias ya utilizado","register")
            is_valid= False
        if len(user['password']) < 8:
            flash("La contraseña debe tener al menos ocho caracteres", "register")
            is_valid= False
